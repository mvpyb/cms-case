...
export default {
  ...
  methods : {
    ...
    async getNavList() {
      ...
      console.log( 'currentNav', that.navList )
    ...
    async showCreateArticle() {
      ...
      if ( [6, 7, 8].indexOf( this.contents.currentNav ) >= 0 && this.contentList.length >= 1 ) {
    ...
    async previewArticle( params ) {
      ...
      // console.log( 'currentNav', currentNav, dev.case )
      // return false
      try {
        ...
        if ( params.status === 1 ) {
          ...
          let url = ''
          // 判断当前页面  如果是 人才招聘 就跳转到 'https://webapps-fat.shadowcreator.com/a/test/#/about/jobDetail?id=1'
          //                                否则 'https://webapps-fat.shadowcreator.com/a/test/#/about/index'
          if ( envStr === 'dev' ) {
            if ( currentNav == 10 ) {
              // 人才招聘
              url = caseUrl + '/#/about/jobDetail?id=' + id
            } else if ( currentNav == 6 ) {
              // 协会简介
              url = caseUrl + '/#/about/index'
            } else if ( currentNav == 7 ) {
              // 协会章程
              url = caseUrl + '/#/about/rule'
            } else if ( currentNav == 8 ) {
              // 阻止架构
              url = caseUrl + '/#/about/organization'
            } else {
              url = caseUrl + '/#/about/index'
            }
          } else {
