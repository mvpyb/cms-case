...
<template>
  <div class="app-container">
    ...
    <div class="tab-headers clearfix mb20">
      <el-button type="primary" class="fl" @click="showCreateArticle">创建文章</el-button>
    ...
    >
      ...
      <el-table-column label="发布时间" width="120">
        <template slot-scope="scope">{{ scope.row.publish_time | changeTime }}</template>
      </el-table-column>
...
export default {
    ...
    return {
      ...
      contentList : [
        ...
        },
        {
          ...
          nav : 'about/rule',
          navName : '协会章程',
          title : '文章标题 about/rule',
          description : '文章描述 about/rule'
        }
      ],
      contents : {
        pid : 4,
  ...
  methods : {
    ...
    async getArticleList( data ) {
      ...
      params.nav = that.contents.currentNav || 0
      ...
      setTimeout( () => {
        ...
        that.contents.total = that.contentList.length
      }, 1000 )
    ...
    async showEditArticle( data ) {
      // this.articleDialogOff = true
      // this.articleDialogType = 'update'
      // this.currentSelectArticle = { ...data }
    ...
    async previewArticle( data ) {
      try {
        ...
        const response = await previewArticle( {
          id : this.currentSelectArticle.id
        } )
        this.loading = false
        await this.initPage()
        await this.getArticleList()
        if ( data.status === 1 ) {
          ...
          let url = ''
          if ( envStr === 'dev' ) {
            url = 'https://kstest.shadowcreator.com/fat/www/YC-1047/news/details.html?id=' + data.id
          } else {
            if ( envStr === 'fat' ) {
              url = 'https://kstest.shadowcreator.com/fat/www/YC-1047/news/details.html?id=' + data.id
            } else if ( envStr === 'uat' ) {
              url = 'https://kstest.shadowcreator.com/uat/www/news/details.html?id=' + data.id
            } else {
              url = 'https://www.shadowcreator.com/news/details.html?id=' + data.id
            }
          }
          window.open( url, '_blank' )
        } else {
          const { detail, dict } = response.data
...
export default {
  ...
  methods : {
    ...
    async getArticleList( payload ) {
      ...
      that.contents.loading = true
      if ( payload ) {
        that.banners.listQuery.page = payload.page
        that.banners.listQuery.limit = payload.limit
      }
      const params = {
    ...
    async previewArticle( params ) {
      ...
      try {
        ...
        if ( params.status === 1 ) {
          ...
          let url = ''
          // if ( envStr === 'dev' ) {
          //   url = 'https://webapps-fat.shadowcreator.com/a/test/#/feature/details?id=' + id
          // } else {
          //   url = 'https://webapps-fat.shadowcreator.com/a/test/#/feature/details?id=' + id
          // }
          if ( envStr === 'dev' ) {
            url = caseUrl + '/#/feature/details?id=' + id
            url = caseUrl + '/#/feature/details?id=' + id
          } else {
            url = caseUrl + '/#/dynamic/details?id=' + id
          }
          window.open( url, '_blank' )
        } else {
